...
describe('clipboard — pasteFromText auto-links whole-cell URLs', () => {
  ...
  it('sets fmt.hyperlink for URL cells, leaves plain text alone', () => {
    cb.pasteFromText('https://trivena.io/\tplain text\nfrappe.io\t42', 'A1', null)
    const f = formats._store()
    expect(sheet._store().A1).toBe('https://trivena.io/')
    expect(f.A1).toEqual({ hyperlink: 'https://trivena.io/' })
    expect(f.B1).toBeUndefined()
    expect(f.A2).toEqual({ hyperlink: 'https://trivena.io' })   // bare domain normalized
    expect(f.B2).toBeUndefined()
  })

  it('tiled single-URL paste links every destination cell', () => {
    cb.pasteFromText('www.trivena.io', 'A1', null, { r0: 0, c0: 0, r1: 1, c1: 0 })
    const f = formats._store()
    expect(f.A1).toEqual({ hyperlink: 'https://www.trivena.io' })
    expect(f.A2).toEqual({ hyperlink: 'https://www.trivena.io' })
  })

  it('does not link when no formats engine is wired (headless paste)', () => {
    const bare = createClipboard({ sheet: makeSheet() })
    expect(bare.pasteFromText('https://trivena.io/', 'A1', null)).toBe(true)
  })
