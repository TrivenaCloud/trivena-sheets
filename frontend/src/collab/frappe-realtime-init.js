// Minimal `trivena.realtime` shim for the public www/sheets page.
//
...
// `window.trivena.realtime` the legacy collab path has no transport, so
...
/**
 * Initialise `window.trivena.realtime` if it isn't already populated.
 *
 * @param {object} [boot]   - the trivena.boot shim — defaults to window.trivena.boot
...
export function ensureFrappeRealtime(boot, opts = {}) {
	...
	w.frappe = w.frappe || {}
	if (w.trivena.realtime && typeof w.trivena.realtime.on === 'function') {
		return w.trivena.realtime
	}
	...
	// itself — `window.trivena.realtime.socket.on('connect_error', ...)`
