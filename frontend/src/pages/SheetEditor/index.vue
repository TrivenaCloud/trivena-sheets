...
function onAiSettingsSaved(s) {
  ...
  if (window.frappe?.boot) window.trivena.boot.ai_assist_enabled = on
  aiEnabled.value = on
}
...
let _sheetTabs = null
const { isSaving, saveError, canWrite, isPublic, isPublicWrite, sheetOwner, loadError, loadSheet, autoCreate, saveExisting, retrySave } =
  usePersistence({
...
function _setupGridInstance() {
  grid = createGrid(canvasRef.value, {
    ...
    onCommit(id, value) {
      ...
      // pre-existing "trivena.io" data) still picks up its link.
