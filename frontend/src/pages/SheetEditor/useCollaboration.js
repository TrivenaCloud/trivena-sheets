...
// the legacy `trivena.realtime` relay path runs unchanged so a deploy of this
...
//   1. `window.trivena.boot.collab_v2`  — server-side flag from site_config
...
export function useCollaboration({
  ...
  function _start(docId) {
    ...
    if (_collabV2Enabled()) {
      ...
      // Legacy path — seed locally, relay through trivena.realtime.
      hydrateYDoc(_doc, { sheet: sheet?.snapshot?.() })
      // The www/sheets page doesn't load Frappe's socketio_client,
      // so `window.trivena.realtime` is undefined and the legacy path
