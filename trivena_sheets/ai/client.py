...
def generate_actions(prompt: str, context: dict, api_key: str, model: str) -> list:
	...
	try:
		...
		trivena.throw("The Anthropic API key is invalid or has been revoked.")
	except anthropic.RateLimitError:
		trivena.throw("The AI service is rate-limited right now. Try again in a moment.")
	except anthropic.APIStatusError as e:
		# Never echo the exception body — it can include request details.
		trivena.throw(f"The AI request failed (HTTP {e.status_code}).")
	except anthropic.APIConnectionError:
		trivena.throw("Could not reach the AI service. Check the server's network access.")
