...
`trivena.client.get_list` / `trivena.client.get` endpoints would let any
...
def can_write_sheet(name: str, user: str | None = None) -> bool:
	...
	Write access comes from **either**:
	  ...
	  * the normal permission system — owner, a granting role, or an explicit
	    per-user DocShare with ``write`` (``trivena.has_permission``); **or**
	...
	if trivena.has_permission("Sheet", doc=name, ptype="write", user=user, throw=False):
		return True
	if user == "Guest":
		return False
...
def assert_can_write_sheet(name: str, user: str | None = None) -> None:
	...
	if not can_write_sheet(name, user):
		trivena.throw(
			trivena._("No write access to sheet {0}").format(name),
...
def _scope_to_readable_sheets(table_prefix: str, user: str | None) -> str:
	...
	if _is_privileged(user):
		return ""
	user_lit = trivena.db.escape(user)
...
def _child_has_permission(doc, ptype: str, user: str | None) -> bool:
	...
	return bool(
		trivena.has_permission("Sheet", doc=sheet_name, ptype=parent_ptype, user=user)
	)
...
def _is_privileged(user: str) -> bool:
	if user == "Administrator":
		return True
	return bool(_PRIVILEGED_ROLES.intersection(trivena.get_roles(user)))
