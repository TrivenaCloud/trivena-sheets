...
def allocate(sheet: str, count: int = 1) -> int:
	...
	"""
	if count < 1:
		trivena.throw("allocate(count) requires count >= 1")
...
def peek(sheet: str) -> int:
	...
	row = trivena.db.get_value("Sheet Seq", sheet, "next_seq")
...
def _ensure_row(sheet: str) -> None:
	...
	if trivena.db.exists("Sheet Seq", sheet):
		return
	try:
		trivena.get_doc({"doctype": "Sheet Seq", "sheet": sheet, "next_seq": 1}).insert(
