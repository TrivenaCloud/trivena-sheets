...
from trivena_sheets.versioning import timeline as tl
...
class BadCursorRejected(unittest.TestCase):
	def test_garbage_string_throws(self):
		# `trivena.throw` raises a generic exception we can match by message.
		with mock.patch(
			"sheets.versioning.timeline.trivena.throw",
