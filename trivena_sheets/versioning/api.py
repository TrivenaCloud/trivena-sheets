...
def save_snapshot(sheet: str, label: str = "") -> dict:
	trivena.has_permission("Sheet", doc=sheet, ptype="write", throw=True)
	clean = (label or "").strip()
	if not clean:
		trivena.throw("Label is required for a named snapshot")
...
def restore(snapshot: str) -> dict:
	snap = trivena.db.get_value("Sheet Snapshot", snapshot, "sheet")
	if not snap:
		trivena.throw(f"Snapshot {snapshot} not found")
	trivena.has_permission("Sheet", doc=snap, ptype="write", throw=True)
...
def head(sheet: str) -> dict:
	trivena.has_permission("Sheet", doc=sheet, throw=True)
