...
def list_buckets(sheet: str, tz_offset_minutes: int = 0, bucket_limit: int = 20) -> dict:
	...
	head_snapshot = trivena.db.get_value("Sheet", sheet, "head_snapshot")
...
def list_bucket_page(
	...
	if bucket not in ALL_BUCKETS:
		trivena.throw(f"Unknown bucket: {bucket}")
	trivena.has_permission("Sheet", doc=sheet, throw=True)
...
def _query_dual_bound(
	...
	rows = trivena.db.sql(sql, params, as_dict=True)
	return _shape(rows, limit)
