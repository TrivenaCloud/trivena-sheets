	...
	snap = trivena.get_doc(
		{
			...
			"actor": actor or trivena.session.user,
...
def _should_snapshot(head_seq: int, last: dict | None) -> bool:
	...
	if ops_since >= ops_thresh:
		return True
	last_at = trivena.utils.get_datetime(last["creation"])
...
def stored_size(snap_name: str) -> int:
	...
	stored = trivena.db.get_value("Sheet Snapshot", snap_name, "sheets_data")
