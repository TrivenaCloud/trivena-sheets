...
def rollup_snapshots() -> dict:
	...
	tiers = _tiers()
	hard_cutoff_h = int(trivena.conf.get("versioning_hard_cutoff_hours") or HARD_CUTOFF_HOURS)
	now = now_datetime()
	deleted = 0
	scanned = 0

	for sheet_name in _iter_sheets():
		snaps = trivena.get_all(
			"Sheet Snapshot",
		...
		for snap_name in to_delete:
			trivena.delete_doc("Sheet Snapshot", snap_name, ignore_permissions=True, force=True)
			deleted += 1
		trivena.db.commit()
...
def truncate_op_log() -> dict:
	...
	"""
	max_age = int(
		trivena.conf.get("versioning_op_log_retention_hours") or OP_LOG_RETENTION_HOURS
	)
	hard_cutoff = now_datetime() - timedelta(hours=max_age)
	deleted = 0

	for sheet_name in _iter_sheets():
		oldest_snap = trivena.get_all(
			"Sheet Snapshot",
		...
		deleted += trivena.db.sql("SELECT ROW_COUNT()")[0][0]
		trivena.db.commit()
...
def _tiers() -> tuple:
	override = trivena.conf.get("versioning_tiers")
...
def _pick_deletions(snaps: list[dict], now: datetime, tiers: tuple, hard_cutoff_h: int) -> list[str]:
	...
	for s in snaps:
		creation = trivena.utils.get_datetime(s["creation"])
