...
from trivena_sheets.permissions import assert_can_write_sheet
from trivena_sheets.sheets.doctype.sheet.storage import (
...
def _insert_new(
	...
	doc = trivena.new_doc("Sheet")
	doc.title = title
	doc.sheets_data = encoded
	doc.head_seq = 0
	doc.insert()
	head_seq = _append_ops_and_save(doc.name, ops_list, byte_size, save_op_type="create")
	trivena.db.set_value("Sheet", doc.name, "head_seq", head_seq, update_modified=False)
...
def _append_ops_and_save(
	...
	for row in rows:
		trivena.get_doc(row).insert(ignore_permissions=True)
	return save_seq
...
def append_op(sheet: str, op: dict) -> int:
	...
	trivena.get_doc(_op_doc(sheet, new_seq, op, trivena.session.user)).insert(
...
def _op_doc(sheet: str, seq: int, op: dict, actor: str) -> dict:
	...
	if not op_type:
		trivena.throw("op_type is required")
	return {
		"doctype": "Sheet Op Log",
		"sheet": sheet,
		"seq": seq,
...
def _coerce_ops(ops) -> list[dict]:
	...
	if isinstance(ops, str):
		try:
			...
			trivena.throw("ops must be a JSON array")
	if not isinstance(ops, list):
		trivena.throw("ops must be a JSON array")
	if len(ops) > MAX_OPS_PER_SAVE:
		trivena.throw(f"Too many ops in one save (max {MAX_OPS_PER_SAVE})")
	return ops


def _validate_payload(sheets_data: str) -> str:
	if not isinstance(sheets_data, str):
		trivena.throw("sheets_data must be a JSON string")
	...
	try:
		...
		trivena.throw("sheets_data is not valid JSON")
	return plain
