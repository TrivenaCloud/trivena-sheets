...
from trivena_framework.utils import get_datetime, now_datetime
...
def retention_days() -> int:
	configured = trivena.conf.get("sheets_trash_retention_days")
...
def hard_delete_sheet(name: str) -> None:
	...
	"""
	trivena.db.set_value("Sheet", name, "head_snapshot", None, update_modified=False)
	trivena.db.delete("Sheet Snapshot", {"sheet": name})
	trivena.db.delete("Sheet Op Log",   {"sheet": name})
	trivena.db.delete("Sheet Seq",      {"sheet": name})
	trivena.delete_doc("Sheet", name, ignore_permissions=True)
...
def purge_trashed_sheets() -> dict:
	...
	for name in expired:
		...
		row = trivena.db.get_value("Sheet", name, ["trashed", "trashed_on"], as_dict=True)
