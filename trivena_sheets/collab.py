...
from trivena_sheets.permissions import can_write_sheet
...
def check_collab_access(name: str) -> dict:
	...
	if trivena.session.user == "Guest":
		trivena.throw("Login required", trivena.AuthenticationError)

	can_read = bool(trivena.has_permission("Sheet", doc=name, ptype="read", throw=False))
	if not can_read:
		...
		can_read = bool(trivena.db.get_value("Sheet", name, "is_public"))
	if not can_read:
...
def load_collab_state(name: str) -> dict:
	...
	if not trivena.db.exists("Sheet Collab State", name):
...
def persist_collab_state(name: str, ydoc_state: str, byte_size: int = 0) -> dict:
	...
	if not trivena.db.exists("Sheet", name):
		trivena.throw(f"Sheet {name!r} does not exist", trivena.DoesNotExistError)

	byte_size = int(byte_size or 0)
	now = trivena.utils.now()

	if trivena.db.exists("Sheet Collab State", name):
		...
		doc = trivena.new_doc("Sheet Collab State")
		doc.sheet = name
...
def _require_collab_secret() -> None:
	...
	"""
	expected = (trivena.conf.get("collab_server_secret") or "").strip()
	if not expected:
		trivena.throw(
			"Collab server secret is not configured "
			"(set `collab_server_secret` in site_config.json)",
			trivena.AuthenticationError,
		)
	sent = (trivena.get_request_header(_COLLAB_SECRET_HEADER) or "").strip()
	...
	if not sent or not hmac.compare_digest(expected, sent):
		trivena.throw("Invalid collab server credentials", trivena.AuthenticationError)
