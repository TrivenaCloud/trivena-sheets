...
from trivena_sheets.sheets.doctype.sheet.cell_codec import cell_map
...
class CellMapCompact(unittest.TestCase):
	def test_reads_compact_v2_slice(self):
		...
		slice_ = {
			"v": 2,
			"current": "Sheet1",
			"trivena_sheets": {"Sheet1": {"rows": {"0": ["Date", "Day"], "1": ["2013-11-26", "26"]}}},
		}
		self.assertEqual(
...
class CellMapLegacy(unittest.TestCase):
	...
	def test_garbage_inputs_yield_empty(self):
		...
		self.assertEqual(cell_map({"trivena_sheets": {"Sheet1": "nope"}}, "Sheet1"), {})
