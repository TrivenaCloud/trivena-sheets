...
``trivena.throw`` is monkey-patched to raise so the module's exception
...
from trivena_sheets.sheets.doctype.sheet import storage
...
class GzipBombGuards(unittest.TestCase):
	...
	def test_compressed_size_over_cap_rejected(self):
		...
		with mock.patch("trivena_framework.throw", side_effect=self._throw):
			with self.assertRaises(ValueError):
				storage.decode_sheets_data(env)
