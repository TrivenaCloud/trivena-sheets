...
class _ListSheetsBase(unittest.TestCase):
	def setUp(self):
		...
		self.trivena.session.user = ME
		# The module calls trivena.utils.cint for clamping — give the mock a
		# real implementation so the arithmetic works.
		self.trivena.utils.cint.side_effect = _cint
		self.trivena.utils.now.return_value = "2026-07-23 12:00:00.000001"
		self.rows = [
		...
		self.trivena.get_list.side_effect = [self.rows, [{"total": 42}]]
	...
	def rows_kwargs(self):
		return self.trivena.get_list.call_args_list[0].kwargs

	def count_kwargs(self):
		return self.trivena.get_list.call_args_list[1].kwargs
...
class Defaults(_ListSheetsBase):
	...
	def test_response_shape_and_is_owner(self):
		...
		self.assertTrue(res["trivena_sheets"][0]["is_owner"])
		self.assertFalse(res["trivena_sheets"][1]["is_owner"])
		self.assertIs(res["trivena_sheets"][1]["is_public"], True)
...
class OrderBy(_ListSheetsBase):
	...
	def test_unknown_order_by_falls_back_to_default(self):
		...
		for call in self.trivena.get_list.call_args_list:
