...
security audit: the stock `trivena.client.get_list` reading Sheet Op Log /
...
side effects, so we mock `trivena.session.user`, `trivena.get_roles`,
`trivena.has_permission`, and `trivena.db.escape` to keep the tests
hermetic.
"""
...
from trivena_sheets import permissions
