...
from trivena_sheets import boot as _boot  # noqa: F401  — see test_collab.py
...
class ExtendBootinfo(unittest.TestCase):
	...
	def test_defaults_to_legacy_path_when_unset(self):
		from trivena_sheets import boot
	...
	def test_picks_up_site_config_values(self):
		...
		self.trivena.conf.get.side_effect = lambda key: {
			"collab_v2": True,
	...
	def test_collab_v2_is_truthy_coerced_to_bool(self):
		...
		self.trivena.conf.get.side_effect = lambda key: {"collab_v2": 1}.get(key)
		b = self._bootinfo()
		boot.extend_bootinfo(b)
