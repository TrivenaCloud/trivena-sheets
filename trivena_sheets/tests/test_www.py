...
class AssetManifestResolution(unittest.TestCase):
    ...
    def setUp(self):
        # Each test gets its own tmpdir + a trivena.get_app_path that
        ...
        self.manifest_dir = os.path.join(
            self.tmp.name, "public", "trivena_sheets", ".vite"
        )
        ...
        self.trivena.get_app_path.return_value = self.tmp.name
        ...
        from trivena_sheets.www import trivena_sheets as www
    ...
    def test_resolves_hashed_paths_from_manifest(self):
        ...
        self.assertEqual(paths["js"], "/assets/trivena_sheets/sheets/index.ABCDEF.js")
        self.assertEqual(paths["css"], ["/assets/trivena_sheets/sheets/index.GHIJKL.css"])
    ...
    def test_falls_back_to_legacy_filenames_when_manifest_is_missing(self):
        ...
        paths = www._asset_paths()
        self.assertEqual(paths["js"], "/assets/trivena_sheets/sheets/index.js")
        self.assertEqual(paths["css"], ["/assets/trivena_sheets/sheets/index.css"])
...
class BootContext(unittest.TestCase):
    ...
    `window.trivena.{session,boot}`. The SPA branches on these for the
    ...
    def setUp(self):
        ...
        self.trivena.session.sid  = "sid-xyz"
        self.trivena.db.get_value.side_effect = (
        ...
        self.trivena.local.site = "test.localhost"
        self.trivena.conf.get.side_effect = lambda key, default=None: {
            "collab_v2":     0,
            "collab_ws_url": None,
            "socketio_port": 9001,
        }.get(key, default)
        self.trivena.sessions.get_csrf_token.return_value = "csrf-token"
    ...
    def test_guest_is_redirected_to_login(self):
        ...
        self.trivena.Redirect = RuntimeError  # any exception we can catch
        from trivena_sheets.www import trivena_sheets as www

        with self.assertRaises(RuntimeError):
            www.get_context(self.ctx)
        self.assertEqual(
            self.trivena.local.flags.redirect_location,
