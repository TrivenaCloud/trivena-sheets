...
from trivena_sheets.permissions import assert_can_write_sheet, can_write_sheet
from trivena_sheets.sheets.doctype.sheet.cell_codec import cell_map as unpack_cell_map
from trivena_sheets.sheets.doctype.sheet.storage import decode_sheets_data
from trivena_sheets.versioning import save as save_mod
...
def ping_presence(name: str) -> None:
	...
	trivena.has_permission("Sheet", doc=name, throw=True)
...
def broadcast_op(name: str, op: str) -> None:
	...
	trivena.publish_realtime(
		"sheet_op",
		{"sheet": name, "user": trivena.session.user, "op": op},
		after_commit=False,
	)
...
def yjs_relay(name: str, event: str, payload: str) -> None:
	...
	if event not in _YJS_EVENTS:
		trivena.throw(f"Unknown yjs event: {event}")
	if event in _YJS_WRITE_EVENTS:
		assert_can_write_sheet(name)
	elif not trivena.has_permission("Sheet", doc=name, ptype="read", throw=False) and not trivena.db.get_value(
		"Sheet", name, "is_public"
	):
		trivena.throw(f"No read access to sheet {name}", trivena.PermissionError)
	trivena.publish_realtime(
		event,
		{"sheet": name, "user": trivena.session.user, "payload": payload},
		after_commit=False,
	)
...
def get_sheet_shares(name: str) -> list:
	...
	for row in rows:
		...
		row.update(identity)
		row["user_image"] = trivena.db.get_value("User", row["user"], "user_image") or ""
	return rows
...
def set_sheet_public(name: str, public: int = 0, write: int = 0) -> dict:
	...
	expose a sheet.
	"""
	trivena.has_permission("Sheet", doc=name, ptype="share", throw=True)
	is_public = 1 if int(public or 0) else 0
	public_write = 1 if (is_public and int(write or 0)) else 0
	trivena.db.set_value("Sheet", name, {"is_public": is_public, "public_write": public_write})
...
def share_sheet(name: str, user: str = "", write: int = 0) -> dict:
	...
	enabled = trivena.db.get_value("User", user, "enabled")
	if enabled is None:
		trivena.throw(f"User {user} not found")
	if not enabled:
		trivena.throw(f"User {user} is disabled")
	...
	trivena.share.add("Sheet", name, user, write=int(write), share=0, notify=False)
...
def _notify_sheet_shared(sheet_name: str, recipient: str, can_edit: bool) -> None:
	...
	Two surfaces:
	  ...
	  * **Email** — only if the site has SMTP configured. `now=False`
	    ...
	    URL using `trivena.utils.get_url` so it works in dev (localhost)
	...
	"""
	try:
		share_doc = trivena.get_doc("Sheet", sheet_name)
		title = share_doc.title or "Untitled Spreadsheet"
		sharer = trivena.db.get_value("User", trivena.session.user, "full_name") or trivena.session.user
		...
		link = f"{trivena.utils.get_url()}/sheets?id={sheet_name}"
		subject = f"{sharer} shared a sheet with you"
		# Frappe's Notification Log surfaces in the bell-icon dropdown.
		trivena.get_doc({
			"doctype": "Notification Log",
			"subject": (
				f"{trivena.utils.escape_html(sharer)} shared the sheet "
				f"<b>{trivena.utils.escape_html(title)}</b> with you "
				f"(can {role})"
			),
			"for_user": recipient,
			"type": "Share",
			...
			"from_user": trivena.session.user,
			"email_content": (
				f"<p>{trivena.utils.escape_html(sharer)} shared the sheet "
				f"<b>{trivena.utils.escape_html(title)}</b> with you. "
				f"You can {role} it.</p>"
		...
		trivena.log_error(title="Sheet share notification failed")


@trivena.whitelist()
def unshare_sheet(name: str, user: str = "") -> dict:
	trivena.has_permission("Sheet", doc=name, ptype="share", throw=True)
	trivena.share.remove("Sheet", name, user)
	return {"status": "ok"}
...
def list_sheets(
	...
	# window.trivena.session — the client can't know who it is on its own.
	me = trivena.session.user
	start = max(trivena.utils.cint(start), 0)
	limit = min(max(trivena.utils.cint(limit) or 50, 1), 100)
	...
	# keeps the owner + DocShare conditions that trivena.db.count would drop.
	# Dict field syntax: Frappe 17 rejects string SQL functions in SELECT.
	total = trivena.get_list(
		"Sheet",
		filters=filters,
	...
	return {"trivena_sheets": rows, "total": total, "now": str(trivena.utils.now())}
...
def get_sheet(name: str, compressed: int = 0) -> dict:
	...
	# `trivena.get_doc` does NOT check read permission by itself, so without
	# this guard any caller who knows a sheet id could exfiltrate its contents.
	access = trivena.db.get_value("Sheet", name, ["is_public", "public_write"], as_dict=True) or {}
	is_public = bool(access.get("is_public"))
	if not is_public:
		trivena.has_permission("Sheet", doc=name, throw=True)
	doc = trivena.get_doc("Sheet", name)
	# A trashed sheet must not open from a bookmarked/shared ?id= link — it's
	# "deleted" as far as the app is concerned until restored.
	if doc.trashed:
		trivena.throw("This sheet is in the trash.", trivena.DoesNotExistError)
	...
	can_write = trivena.session.user != "Guest" and can_write_sheet(name)
	return {
		"name": doc.name,
		"title": doc.title,
		"sheets_data": raw if trivena.utils.cint(compressed) else decode_sheets_data(raw),
		"is_public": is_public,
...
def delete_sheet(name: str) -> str:
	...
	trivena.has_permission("Sheet", doc=name, ptype="delete", throw=True)
	trivena.db.set_value(
		"Sheet",
		name,
		{"trashed": 1, "trashed_on": trivena.utils.now_datetime(), "trashed_by": trivena.session.user},
...
def delete_sheet_permanent(name: str) -> str:
	...
	if not trivena.db.get_value("Sheet", name, "trashed"):
		trivena.throw("Only sheets in the trash can be permanently deleted.")
	from trivena_sheets.trash import hard_delete_sheet
...
def list_trash() -> dict:
	...
	from trivena_sheets.trash import retention_days

	sheets = trivena.get_list(
		"Sheet",
		filters={"trashed": 1, "owner": trivena.session.user},
		fields=["name", "title", "trashed_on"],
		order_by="trashed_on desc",
		limit=100,
	)
	return {"trivena_sheets": sheets, "retention_days": retention_days()}
...
def rename_sheet(name: str, title: str) -> str:
	...
	if not title:
		trivena.throw("Title is required")
	doc = trivena.get_doc("Sheet", name)
	doc.title = title
	doc.save()
	return doc.name
...
def duplicate_sheet(name: str) -> str:
	...
	trivena.has_permission("Sheet", doc=name, throw=True)
	src = trivena.get_doc("Sheet", name)
...
def get_ai_settings() -> dict:
	...
	doc = trivena.get_cached_doc(AI_SETTINGS)
	return {
...
def save_ai_settings(api_key: str = "", enabled: int = 0, model: str = "") -> dict:
	...
	if "System Manager" not in trivena.get_roles():
		trivena.throw("Not permitted to change AI settings", trivena.PermissionError)
	doc = trivena.get_doc(AI_SETTINGS)
	...
	trivena.clear_document_cache(AI_SETTINGS, AI_SETTINGS)
...
def ai_assist(name: str, prompt: str, selection: str) -> dict:
	...
	if not prompt:
		trivena.throw("Type what you'd like to do first.")
	if len(prompt) > MAX_PROMPT_LEN:
		trivena.throw("That request is too long.")

	cfg = trivena.get_cached_doc(AI_SETTINGS)
	if not cfg.enabled:
		trivena.throw("AI Assist isn't enabled for this site.")
	model = cfg.model or DEFAULT_AI_MODEL

	sel = trivena.parse_json(selection) if selection else {}
	if not isinstance(sel, dict):
		sel = {}
	sheet_name = sel.get("sheet") or "Sheet1"

	data = json.loads(decode_sheets_data(trivena.get_doc("Sheet", name).sheets_data) or "{}")
	cell_map = unpack_cell_map((data or {}).get("sheet") or {}, sheet_name)

	from trivena_sheets.ai import context as ai_context
	from trivena_sheets.ai import heuristics as ai_heuristics
	from trivena_sheets.ai import validate as ai_validate
	...
	if raw is None:
		if model.strip().lower() in ("mock", "demo"):
			...
			key = _ai_key(cfg)
			if not key:
				trivena.throw("No Anthropic API key is configured.")
			from trivena_sheets.ai import client as ai_client
...
def _user_identity(user: str) -> dict:
	...
	full_name = trivena.db.get_value("User", user, "full_name") or user
	user_image = trivena.db.get_value("User", user, "user_image") or ""
