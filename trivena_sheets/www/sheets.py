...
_ASSET_BASE = "/assets/trivena_sheets/sheets/"
_ASSET_CACHE = None
...
def _read_asset_manifest() -> dict:
    ...
    path = os.path.join(
        trivena.get_app_path("trivena_sheets"),
        "public", "trivena_sheets", ".vite", "manifest.json",
    )
    try:
...
def _guest_may_view(sheet_id) -> bool:
    ...
    if not sheet_id:
        return False
    try:
        return bool(trivena.db.get_value("Sheet", sheet_id, "is_public"))
    except Exception:
        return False


def get_context(context):
    if trivena.session.user == "Guest" and not _guest_may_view(trivena.form_dict.get("id")):
        ...
        trivena.local.flags.redirect_location = "/login?redirect-to=/sheets"
        raise trivena.Redirect
    ...
    context.csrf_token = trivena.sessions.get_csrf_token()
    # The SPA reads `window.trivena.session.{user,user_fullname}` for the
    ...
    context.session_user_fullname = trivena.db.get_value("User", user, "full_name") or user
    context.session_user_image = trivena.db.get_value("User", user, "user_image") or ""

    # `window.trivena.boot.*` carries the collab v2 feature flag and the
    # session id the Hocuspocus provider passes as its auth token. Without
    # this, the SPA falls back to the legacy `trivena.realtime` relay path.
    ...
    context.collab_v2     = bool(trivena.conf.get("collab_v2") or False)
    context.collab_ws_url = trivena.conf.get("collab_ws_url") or None
    context.session_sid   = getattr(trivena.session, "sid", "") or ""

    # Sitename + socketio_port let the SPA stand up its own `trivena.realtime`
    ...
    context.sitename       = trivena.local.site
    context.socketio_port  = trivena.conf.get("socketio_port") or 9000
    ...
    context.ai_assist_can_configure = "System Manager" in trivena.get_roles()
    try:
        ai = trivena.get_cached_doc("Sheets AI Settings")
    ...
    # frontend/src/utils/sentry.js reads these off window.trivena.boot
    # and short-circuits when the DSN is empty.
    context.sentry_dsn         = trivena.conf.get("sheets_sentry_dsn") or ""
    context.sentry_environment = (
        trivena.conf.get("sheets_sentry_environment")
        or ("development" if trivena.conf.get("developer_mode") else "production")
    )
    context.sentry_release = trivena.conf.get("sheets_sentry_release") or ""
