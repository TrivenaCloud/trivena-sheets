...
def execute():
	sheet_names = trivena.get_all("Sheet", pluck="name")
	if not sheet_names:
		return
...
def _migrate_one(name: str) -> bool:
	...
	if trivena.db.exists("Sheet Cell", {"parent_sheet": name}):
		return False
	...
	sheets   = (parsed.get("sheet")   or {}).get("trivena_sheets", {}) or {}
	...
	for sheet_name, cells in sheets.items():
		for cell_id, raw in cells.items():
			...
			rows.append((
				trivena.generate_hash(length=10),
				name,
				sheet_name,
				cell_id,
				str(raw),
