...
@trivena.whitelist(methods=["GET", "POST"])
...
def get_link_preview(url: str) -> dict:
	...
	cached = trivena.cache.get_value(cache_key)
	...
	try:
		...
		trivena.logger("trivena_sheets").error(f"link preview failed for {url}", exc_info=True)
		result = {"error": True}
		ttl = CACHE_ERR_SEC

	trivena.cache.set_value(cache_key, result, expires_in_sec=ttl)
	return result
	...
	for _ in range(MAX_REDIRECTS + 1):
		...
		try:
			...
			if resp.status in REDIRECT_STATUSES:
				...
				if not location:
					raise trivena.ValidationError("Redirect without location")
				current = urljoin(current, location)
				continue
			if resp.status >= 400:
				raise trivena.ValidationError(f"HTTP {resp.status}")
			if "html" not in resp.headers.get("content-type", ""):
				raise trivena.ValidationError("Not an HTML page")
			return current, resp.read(MAX_BYTES)
		finally:
			pool.close()
	raise trivena.ValidationError("Too many redirects")
...
def _validate_and_resolve(url: str) -> tuple[str, str, int, str, str]:
	...
	if parsed.scheme not in ("http", "https"):
		raise trivena.ValidationError("Only http/https URLs allowed")
	if parsed.port not in (None, 80, 443):
		raise trivena.ValidationError("Non-standard port not allowed")
	host = parsed.hostname
	if not host:
		raise trivena.ValidationError("Invalid URL")
	...
	try:
		...
		raise trivena.ValidationError("Host not resolvable")
	pinned_ip = None
	for info in infos:
		ip = info[4][0]
		...
		if (
			...
			raise trivena.ValidationError("Address not allowed")
